<template>
    <div class="tmk-user-header">
      ...
      <template v-else>
        <div class="tmk-user-header__dropdown">
          <img class="user-avatar" :src="userInfo.avatarUrl" :alt="userInfo.username"/>
          <div class="user-name" :title="userInfo.nickName">{{userInfo.nickName}}</div>
        </div>
        <div class="tmk-user-header__arrow"></div>
        <ul class="tmk-user-header__menu">
          <li class="user-id-item">
            <span> {{$t('menu.userId')}} {{userInfo.username}}}</span>
          </li>
          <li class="feature-item"  v-for="(menuInfo, mIndex) of getSubmenu"
              :key="mIndex">
